...
const Navbar = () => {
  ...
  return (
    <section className="py-2 bg-white">
          ...
          {buttonLinks.map((button) => (
            <Link to={button.to} key={button.name}>
              <button className="flex items-center gap-2 border-r pr-3">
                <img src={button.icon} alt={button.name} className="w-6 h-auto object-cover" />
      ...
      <div
        className={`fixed inset-0 top-14 bg-white z-50 transform ${menuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}
      >
        <div className="container mx-auto flex flex-col items-start px-4 space-y-6 pt-6">
          {navLinks.map((link) => (
            <Link to={link.to} className="text-black text-xl font-semibold" key={link.name}>
              {link.name}
            </Link>
          ))}
          {buttonLinks.map((button) => (
            <Link to={button.to} className="text-black text-xl font-semibold flex items-center gap-2" key={button.name}>
              <img src={button.icon} alt={button.name} className="w-6 h-auto object-cover" />
